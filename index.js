...
function getWinnersByYear(data, getYears, getWinners, ) {
    ...
    for (let i =0; i < data.length; i ++) {
        list.push("In " + years[i] + ", " + winners[i] + " won the world cup!");
    }
...
function getAverageGoals(getFinals) {
   ...
   for (let i = 0; i <finals.length; i++){
       num += finals[i]["Home Team Goals"];
       num += finals[i]["Away Team Goals"];
   }
   num = Math.round(100 * (num/finals.length)) / 100;
   return "" + num;
}
